...
class highwayNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.soc_embedding_size = 48

        ## Define network weights

        # 输入
        self.ip_emb = torch.nn.Linear(3, self.input_embedding_size)
        ...
        if self.use_maneuvers:
            ...
            self.dec_lstm = torch.nn.LSTM(
                self.encoder_size + self.dyn_embedding_size + self.num_lat_classes + self.num_lon_classes,
                self.decoder_size)
        else:
    ...
    def forward(self, hist, nbrs, masks, lat_enc, lon_enc):
        ...
        soc_enc = soc_enc.permute(0, 2, 1).unsqueeze(-1)
        _, (hist_copy, _) = self.enc_lstm(self.leaky_relu(self.ip_emb(hist)))
        soc_enc[:, :, 6, 0] = hist_copy[0]
